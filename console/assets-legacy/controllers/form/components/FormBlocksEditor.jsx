...
export function FormBlocksEditor(props) {
    ...
    // Save helper
    function save(title, description, proposeNewsletter, onlyForMembers, blocks) {
        if (saveTimeout) {
    ...
    const addBlock = (type) => {
        ...
        save(title, description, proposeNewsletter, onlyForMembers, newBlocks);
    };
        ...
        <div className="form-blocks p-4">
            ...
            >
                <MetadataView
                    ...
                    setTitle={(newTitle) => {
                        setTitle(newTitle);
                        save(newTitle, description, proposeNewsletter, onlyForMembers, blocks);
                    }}
                    description={description}
                    setDescription={(newDescription) => {
                        setDescription(newDescription);
                        save(title, newDescription, proposeNewsletter, onlyForMembers, blocks);
                    }}
                    proposeNewsletter={proposeNewsletter}
                    setProposeNewsletter={(newProposeNewsletter) => {
                        setProposeNewsletter(newProposeNewsletter);
                        save(title, description, newProposeNewsletter, onlyForMembers, blocks);
                    }}
                    onlyForMembers={onlyForMembers}
                    setOnlyForMembers={(newOnlyForMembers) => {
                        setOnlyForMembers(newOnlyForMembers);
                        save(title, description, proposeNewsletter, newOnlyForMembers, blocks);
                    }}
                />
            </div>
