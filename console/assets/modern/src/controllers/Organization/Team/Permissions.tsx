...
const CATEGORY_SPECIFIC_PERMISSIONS = [
  "posts",
  "pages",
  "trombinoscope",
  "events",
...
export default function (props: Props) {
  ...
  const initialProjectPermissionsCategories = useMemo(
    () =>
      props.projectsPermissionsCategoriesValue
        ? JSON.parse(props.projectsPermissionsCategoriesValue)
        : {},
    [props.projectsPermissionsCategoriesValue],
  );
  ...
  const initialCategoryScope = useMemo(() => {
    ...
    props.projects.forEach((project) => {
      ...
      CATEGORY_SPECIFIC_PERMISSIONS.forEach((categoryType) => {
        const hasSpecificCategories =
          initialProjectPermissionsCategories[project.uuid]?.[categoryType];
        scope[project.uuid][categoryType] = hasSpecificCategories
          ? "specific"
          : "all";
      });
    });
  ...
  const [categoryScope, setCategoryScope] =
    useState<Record<string, Record<string, "all" | "specific">>>(
      initialCategoryScope,
    );
  ...
  const getCategoriesForProject = (
    projectId: string,
    categoryType: string,
  ): Category[] => {
    switch (categoryType) {
      case "posts":
        return props.postsCategories.filter(
          (cat) => cat.projectId === projectId,
        );
      case "pages":
        return props.pagesCategories.filter(
          (cat) => cat.projectId === projectId,
        );
      case "trombinoscope":
        return props.trombinoscopeCategories.filter(
          (cat) => cat.projectId === projectId,
        );
      case "events":
        return props.eventsCategories.filter(
          (cat) => cat.projectId === projectId,
        );
      default:
        return [];
  ...
  const areAllPermissionsEnabled = (projectId: string): boolean => {
    ...
    return getAllPermissions.every(
      (permission) => projectPerms[permission] === true,
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <input
        type="hidden"
        name={props.projectsPermissionsField}
        value={JSON.stringify(projectPermissions)}
      />
      <input
        type="hidden"
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {Object.keys(props.definitions).map((section) => (
                        <div key={`${project.uuid}-${section}`}>
                          ...
                          <div className="space-y-6">
                            {Object.keys(props.definitions[section]).map(
                              (category) => (
                                <div
                                  className="space-y-1"
                                ...
                                >
                                  ...
                                  {CATEGORY_SPECIFIC_PERMISSIONS.includes(
                                    category,
                                  ) && (
                                    <Collapsible
                                      defaultOpen={
