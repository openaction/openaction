...
class FormController extends AbstractApiController
{
    ...
    public function answer(ValidatorInterface $validator, FormAnswerApiPersister $persister, FormAnswerTransformer $transformer, Request $request, string $id)
    {
        if (!$form = $this->repository->findOneByBase62Uid($id)) {
            throw $this->createNotFoundException();
        }
        ...
        $answer = $persister->persist($form, $data, $linkedContact);

        return $this->handleApiItem($answer, $transformer, Response::HTTP_CREATED);
    }
}
