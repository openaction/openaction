...
use App\Community\Printing\Consumer\RequestCampaignPreflightHandler;
...
class RequestCampaignPreflightHandlerTest extends KernelTestCase
{
    ...
    public function testConsumeInvalid()
    {
        ...
        $handler = static::getContainer()->get(RequestCampaignPreflightHandler::class);

        // Shouldn't fail
        $this->assertTrue($handler(new RequestCampaignPreflightMessage(0)));
    }
    ...
    public function testConsumeUnaddressedNoEnvelopNoQr()
    {
        ...
        // Handle
        $handler = static::getContainer()->get(RequestCampaignPreflightHandler::class);
        $handler(new RequestCampaignPreflightMessage($campaign->getId()));

        // Should have created the request on LGP
        /** @var FilesystemOperator $lgpStorage */
        $lgpStorage = static::getContainer()->get('lgp.storage');
        $this->assertTrue($lgpStorage->fileExists('Preflight/IN/TODO/'.self::UNADDRESSED_NOENVELOP_NOQR.'.zip'));

        $zip = $this->extractZipFile($lgpStorage->read('Preflight/IN/TODO/'.self::UNADDRESSED_NOENVELOP_NOQR.'.zip'));
...
',
        ...
        $this->assertStringEqualsFile(self::FIXTURE_FILE, $zip['source.pdf']);
    }
    ...
    public function testConsumeUnaddressedEnvelopedUniqueQr()
    {
        ...
        /** @var FilesystemOperator $lgpStorage */
        $lgpStorage = static::getContainer()->get('lgp.storage');
        $this->assertTrue($lgpStorage->fileExists('Preflight/IN/TODO/'.self::UNADDRESSED_ENVELOPED_UNIQUEQR.'.zip'));

        $zip = $this->extractZipFile($lgpStorage->read('Preflight/IN/TODO/'.self::UNADDRESSED_ENVELOPED_UNIQUEQR.'.zip'));
