...
class Citipo implements CitipoInterface
{
    ...
    {
        return $this->parseCollection($this->request('GET', '/api/website/events?page='.$page.'&category='.$category, $apiToken));
    }
    ...
    public function createFormAnswer(string $apiToken, string $id, array $payload, array $authToken = null)
    {
        if ($authToken) {
            return $this->memberRequest('POST', '/api/website/forms/'.$id.'/answer', $apiToken, $authToken, [
                'json' => ['fields' => $payload],
            ], nullable: false);
        }

        return $this->request('POST', '/api/website/forms/'.$id.'/answer', $apiToken, [
            'json' => ['fields' => $payload],
        ], nullable: false);
    }
